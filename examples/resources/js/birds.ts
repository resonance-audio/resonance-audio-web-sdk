...
function integrateBirdPaths(elements) {
    for (let i = 0; i < elements.length - 1; i++) {
        ...
        if (numberOfNeighbors > 0) {
            ...
            elements[i].vz +=
                alignmentWeight * alignment.z +
                cohesionWeight * cohesion.z +
                separationWeight * separation.z;

            let elementVelocityNorm = Math.sqrt(
                elements[i].vx * elements[i].vx +
                    elements[i].vy * elements[i].vy +
                    elements[i].vz * elements[i].vz
            );
            elements[i].vx =
                (elements[i].vx / elementVelocityNorm) * elements[i].speed;
            elements[i].vy =
                (elements[i].vy / elementVelocityNorm) * elements[i].speed;
            elements[i].vz =
                (elements[i].vz / elementVelocityNorm) * elements[i].speed;
        }
        ...
        elements[i].radius =
            (1 - Math.abs(elements[i].z - 0.5) * 2) * radiusRange +
            radiusMinimum;
    }
    canvasControl.draw();
}
...
let onLoad = function () {
    ...
    for (let i = 0; i < numberOfBirds; i++) {
        ...
        elements[i].vz = Math.sin(phaseEle) * elements[i].speed;
    }
    ...
    sourcePlayback.onclick = function (event: MouseEvent) {
        switch ((event.target as HTMLElement).textContent) {
            case "Play":
                {
                    ...
                    (event.target as HTMLElement).textContent = "Pause";
            ...
            case "Pause":
                {
                    (event.target as HTMLElement).textContent = "Play";
